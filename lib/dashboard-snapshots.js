...
function normalizeQualifiedIds(value) {
...
function applyQualityGate(leagues, options = {}) {
  const qualifiedIds = normalizeQualifiedIds(options.qualifiedLeagueIds);
  const requireQualified = options.requireQualified === true;
  const manifestAvailable = Boolean(qualifiedIds && qualifiedIds.size > 0);
  let mode = 'legacy_unqualified';
  let filtered = leagues;

  if (requireQualified && manifestAvailable) {
    mode = 'qualified_only';
    filtered = leagues.filter((league) => {
      const id = String(league.id || league.leagueId || '');
      return Boolean(id && qualifiedIds.has(id)) || league.qaQualified === true || league.siteEligible === true;
    });
  } else if (requireQualified && !manifestAvailable) {
    mode = 'qualified_required_manifest_missing';
    filtered = [];
  } else if (!requireQualified && manifestAvailable) {
    mode = 'manifest_available_not_required';
  }
  ...
  return {
    ...
    meta: {
      mode,
      input: leagues.length,
      output: filtered.length,
      excluded: Math.max(0, leagues.length - filtered.length),
      requireQualified,
      manifestAvailable,
    },
  };
...
function buildDashboardSnapshots(leagues, divCounts = {}, options = {}) {
  ...
  const qualityGate = applyQualityGate(leagues.filter((league) => league.status !== 'template'), options);
  const summaryLeagues = qualityGate.leagues
    .map((league) => ({
      id: league.id,
      name: league.name,
      sport: league.sport,
      ...
      qaQualified: league.qaQualified || false,
      siteEligible: league.siteEligible || false,
